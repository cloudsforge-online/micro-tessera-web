...
/**
 * The smallest window these tests need. `test/hosts.test.ts` carries the same shape; this suite
 * needs it across three separate cases rather than inside one helper, so it is install/remove
 * here rather than a wrapper.
 */
const globals = globalThis as unknown as Record<string, unknown>
let saved: PropertyDescriptor | undefined

function installWindow(url: string): void {
  saved = Object.getOwnPropertyDescriptor(globals, 'window')
...
function removeWindow(): void {
  if (saved) Object.defineProperty(globals, 'window', saved)
